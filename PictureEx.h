...
public:
	...
	BOOL Draw(int mywpstyle,int step);
	CImage *m_image;
	BOOL blankimage;
	BOOL bPngType;
	int m_n_step;
...
afx_msg BOOL OnEraseBkgnd(CDC* pDC);
	...
	void SlidePaintDCBy(CDC& myDC,int xsize,int ysize,int dcx,int dcy,HDC m_hMemDC,int scx,int scy,int srcsizex,int srcsizey,int pmode,int bwpstyle);
	void CreateTempBitmap(CDC* pDC);
	void DeleteTempBitmap();
	void DeleteScreenBitmap();
	void CreateScreenBitmap(CDC* pDC);
	enum GIFBlockTypes GetNextBlock() const;
		void OverOnceSpecialEffect(CPaintDC& dc);
	...
	DWORD m_iStartTime;
	BOOL m_bStartingeffect;//已经开始图片特效
	BOOL m_bShouldStart;//应该开始图片特效
	BOOL m_crossstop;
		CBitmap	m_bmOld;				// old bitmap..
	CDC		m_dcOld;				// ..device context
		CSize		m_szOld;
	CDC		m_dcBlend; // device context
	CBitmap	m_bmBlend; // bitmap

	CDC		m_screenmemdc; // device context
	CBitmap	m_screenbitmap; // bitmap

	// Generated message map functions
	//{{AFX_MSG(CMyStatic)
	afx_msg void OnDestroy();
	afx_msg void OnPaint();
	void FadeOut(CPaintDC& dc);
	void CrossShow(CPaintDC& dc);
	void HorCrossShow(CPaintDC& dc);
	void CenterCrossShow(CPaintDC& dc);
	//}}AFX_MSG
