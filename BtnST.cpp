...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	else // ...else draw non pressed button
	{
		...
		if (m_bIsFlat)//&&!m_bDrawTransparent
		{
...
DWORD CButtonST::SetIcon(LPCTSTR nIconpathIn, LPCTSTR nIconpathOut)
{
	HICON		hIconIn = NULL;
	HICON		hIconOut = NULL;

	
	if (nIconpathIn&&lstrlen(nIconpathIn))
	{
		hIconIn = (HICON)::LoadImage(NULL, nIconpathIn, IMAGE_ICON, 0, 0, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION | LR_LOADFROMFILE);
	...
	else
	{
		...
		hIconIn = BTNST_AUTO_GRAY;

	}
	// Set icon when the mouse is IN the button
	
	// Set icon when the mouse is OUT the button
	if (nIconpathOut&&lstrlen(nIconpathOut))
	{
		
		hIconOut = (HICON)::LoadImage(0, nIconpathOut, IMAGE_ICON, 0, 0, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION | LR_LOADFROMFILE);
	} // if
	else
	{
...
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
	...
	if (hIconIn)
	{
		if (hIconIn == BTNST_AUTO_GRAY)
		{
			hIconIn = CreateGrayscaleIcon(hIconOut);
		} // if
...
//
DWORD CButtonST::SetBitmaps(LPCTSTR nBitmapPathIn, COLORREF crTransColorIn, LPCTSTR nBitmapPathOut, COLORREF crTransColorOut)
{
	HBITMAP		hBitmapIn = NULL;
	HBITMAP		hBitmapOut = NULL;
	
	// Load bitmap In
if (nBitmapPathIn&&lstrlen(nBitmapPathIn))
	hBitmapIn = (HBITMAP)::LoadImage(NULL, nBitmapPathIn, IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION | LR_LOADFROMFILE);

	// Load bitmap Out
if (nBitmapPathOut&&lstrlen(nBitmapPathOut))
		hBitmapOut = (HBITMAP)::LoadImage(NULL, nBitmapPathOut, IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION | LR_LOADFROMFILE);
...
void CButtonST::DrawTransparent(BOOL bTransparent,BOOL bRepaint)
{
